...
let handler  = async (m, { conn, args, usedPrefix, command }) => {
  ...
  if ((args[0] && args[0] == 'plz') || global.conn.user.jid == conn.user.jid) {
    ...
    conn.on('qr', async qr => {
      let scan = await parent.sendFile(m.chat, await qrcode.toDataURL(qr, { scale: 8 }), 'qrcode.png', 'Scan QR code for if you want to be Bot\n\n1.Open whatsapp web\n2.Scan QR code\nQR code Expired within 20s ', m)
      setTimeout(() => {
  ...
  } else throw 'Cannot build bots within bots!\n\nhttps://wa.me/' + global.conn.user.jid.split`@`[0] + '?text=.jadibot'
}
